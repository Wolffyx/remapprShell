...
TestCase {
    ...
    function lineWith(hints) {
        return Bus.notify(["app", 0, "", "summary", "body", [], hints, -1], {}, null);
    }

    // ---- the crash ----------------------------------------------------

    function test_a_byte_array_is_gone_before_parsing() {
        const line = lineWith({ "image-data": { type: "(iiibiiay)", data: [64, 64, 256, true, 8, 4, Bus.pixels(16384)] } });
    ...
    function test_a_large_line_still_parses() {
        for (const n of [262144, 1048576]) {
            const msg = BusLine.parse(lineWith({ "image-data": { type: "(iiibiiay)", data: [1, 1, 1, true, 8, 4, Bus.pixels(n)] } }));
            verify(msg !== null);
    ...
    function test_short_arrays_are_left_alone() {
        const stripped = BusLine.stripByteArrays(lineWith({ "image-data": { type: "(iiibiiay)", data: [64, 48, 256, true, 8, 4, Bus.pixels(16384)] } }));
    ...
    function test_the_threshold_is_where_it_says_it_is() {
        const under = "[" + Bus.pixels(BusLine.byteRunLength - 1).join(",") + "]";
        const over = "[" + Bus.pixels(BusLine.byteRunLength).join(",") + "]";
