...
// Material Design 3 colour roles, worked out from one seed colour.
//
// The shell draws itself in Material's roles -- surfaces, containers, primary,
// outline -- in a light and a dark scheme, and which of the two is on follows
// Plasma: see Theme. This file is only the arithmetic, and pure, so it lives
// in a leaf module of its own where the tests can load it (a module holding one
// Quickshell-dependent singleton cannot be imported by qmltestrunner at all).
//
// Material's tonal palettes are built in HCT, whose "tone" is CIELAB's L*.
// Here they are built in CIELAB's LCh directly: tone is exactly Material's, and
// the hue and chroma are CIELAB's rather than CAM16's -- close enough for a
// panel, and a fraction of the code. A colour that does not exist at the tone
// asked for keeps its hue and gives up chroma until it does.
...
QtObject {
    ...
    function parse(value) {
        ...
        if (m) {
            ...
            return root._clampRgb(root._oklchToLinear(l, parseFloat(m[3]), parseFloat(m[4])));
        }
        return null;
    }
    ...
    function normalise(value, fallback) {
        const rgb = root.parse(value);
        return rgb ? root.hex(rgb) : fallback;
    }

    // "#aarrggbb", which a QML colour accepts. For translucent surfaces.
    function withAlpha(value, alpha) {
        const rgb = root.parse(value) ?? { r: 0, g: 0, b: 0 };
        const a = Math.round(Math.max(0, Math.min(1, alpha)) * 255).toString(16).padStart(2, "0");
        return `#${a}${root.hex(rgb).slice(1)}`;
    }
    ...
    function _clampRgb(lin) {
        return { r: root._fromLinear(Math.max(0, Math.min(1, lin.r))),
                 g: root._fromLinear(Math.max(0, Math.min(1, lin.g))),
                 b: root._fromLinear(Math.max(0, Math.min(1, lin.b))) };
    }

    readonly property real _eps: 216 / 24389
    readonly property real _kappa: 24389 / 27

    function _f(t) { return t > root._eps ? Math.cbrt(t) : (root._kappa * t + 16) / 116; }
    function _finv(t) { const c = t * t * t; return c > root._eps ? c : (116 * t - 16) / root._kappa; }

    function lab(value) {
        const rgb = root.parse(value) ?? { r: 0, g: 0, b: 0 };
        const r = root._toLinear(rgb.r), g = root._toLinear(rgb.g), b = root._toLinear(rgb.b);
        const x = (0.4124564 * r + 0.3575761 * g + 0.1804375 * b) / 0.95047;
        const y = 0.2126729 * r + 0.7151522 * g + 0.0721750 * b;
        const z = (0.0193339 * r + 0.1191920 * g + 0.9503041 * b) / 1.08883;
        const fx = root._f(x), fy = root._f(y), fz = root._f(z);
        return { l: 116 * fy - 16, a: 500 * (fx - fy), b: 200 * (fy - fz) };
    }

    function lch(value) {
        const c = root.lab(value);
    ...
    function _lchToLinear(l, c, h) {
        ...
        const x = root._finv(fx) * 0.95047;
        const y = l > 8 ? Math.pow(fy, 3) : l / root._kappa;
        const z = root._finv(fz) * 1.08883;
    ...
    function tone(hue, chroma, t) {
        ...
        if (root._inGamut(root._lchToLinear(t, hi, hue)))
            return root.hex(root._clampRgb(root._lchToLinear(t, hi, hue)));
        for (let i = 0; i < 24; i++) {
            const mid = (lo + hi) / 2;
            if (root._inGamut(root._lchToLinear(t, mid, hue)))
                lo = mid;
            else
                hi = mid;
        }
        return root.hex(root._clampRgb(root._lchToLinear(t, lo, hue)));
    }
    ...
    function luminance(value) {
        const rgb = root.parse(value) ?? { r: 0, g: 0, b: 0 };
        return 0.2126 * root._toLinear(rgb.r) + 0.7152 * root._toLinear(rgb.g) + 0.0722 * root._toLinear(rgb.b);
    }
    ...
    function seed(accent, systemAccent) {
        const fallback = root.normalise(root.accents.blue, "#4f60c8");
        if (accent === "plasma")
            return root.normalise(systemAccent, fallback);
        if (root.accents[accent] !== undefined)
            return root.normalise(root.accents[accent], fallback);
        return root.normalise(accent, fallback);
    }
    ...
    function scheme(seedColour, dark) {
        const s = root.lch(root.normalise(seedColour, "#4f60c8"));
        const h = s.h;
        const grey = s.c < 6;
        const pc = grey ? s.c : Math.max(s.c, 36);

        const P = t => root.tone(h, pc, t);
        const S = t => root.tone(h, grey ? s.c : 16, t);
        const T = t => root.tone((h + 60) % 360, grey ? s.c : 24, t);
        const N = t => root.tone(h, grey ? 0 : 4, t);
        const V = t => root.tone(h, grey ? 0 : 9, t);
        const E = t => root.tone(32, 70, t);
        const G = t => root.tone(145, 48, t);
        const W = t => root.tone(75, 64, t);
